import { IPricing, IPricingMode } from "@/Utils/types/interface";
import { FC } from "react";
import Button from "@/components/Button";
import { FaCheck } from "react-icons/fa6";

interface IPricingCardProps extends IPricing {
  mode: IPricingMode;
}
...
const PricingCard: FC<IPricingCardProps> = ({
  ...
  return (
    ...
    >
      <figure  className={`absolute w-72 h-60 bg-sky-600 -z-10 rounded-full blur-3xl opacity-20 -top-12 ${
      ...
      <Button variant={isMostPopular ? "primary" : "secondary"} isFontSize>
        buy plan
      </Button>
      <ul className="space-y-4">
        {features.map((feature, index) => (
          <li key={index} className="flex i tems-center gap-4">
